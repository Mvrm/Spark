...
wordCounts = wordPairs.reduceByKey(lambda k,v: add(k,v))
...
wordCountsCollected = (wordsRDD
                       .map(lambda x: (x, 1))
                       .reduceByKey(lambda k,v: add(k,v))
                       .collect())
...
from operator import add
totalCount = (wordCounts.map(lambda (key, value): value)
                        .reduce(add))
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.filter(lambda x: True if x else False)
