...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    val = IDandRatingsTuple[0]
    val1 = IDandRatingsTuple[1]
    length = float(len(val1))
    avg = float(reduce(lambda x, y: x + y, val1)/ length)
    return (val, (length, avg))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD)).map(lambda a : (float(a[1][1][1]),a[1][0],a[1][1][0]))
#.map(lambda a : (a[0][1][1][1],a[0][1][0],[0][1][1][0]))
print 'movieNameWithAvgRatingsRDD: %s\n' % movieNameWithAvgRatingsRDD.take(3)
#[(2049, (u'Happiest Millionaire, The (1967)', (22.0, 3.6818181818181817)))]
#(1.0, u'Autopsy (Macchie Solari) (1975)', 1)
...
def computeError(predictedRDD, actualRDD):
    ...
    sumError = float(totalRDD2.map(lambda ((x1,x2),y): y).reduce(lambda a,b: a+b))
        
    totalError2 = float(totalRDD2.map(lambda ((x1,x2),y): y).count())
    ...
    return float(math.sqrt(sumError/totalError2))
...
print testForPredictingRDD.take(1)
...
testForAvgRDD = testRDD.map(lambda entry: (entry[0],entry[1],trainingAvgRating))
testAvgRMSE = computeError(testRDD, testForAvgRDD)
print 'The RMSE on the average set is %s' % testAvgRMSE
print (abs(testAvgRMSE - 1.12036693569))
...
myRatedMovies = trainingRDD.take(10)
...
print myUnratedMoviesRDD.take(1)
